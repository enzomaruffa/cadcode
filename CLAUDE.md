...
- **Specs (`require(...)`) are ambient** (bound on builtins with `print_hint`, `require_motion`+`turn`/`slide`/`screw`, and `flow_port`+`require_flow`): parts assert their own sanity only; **scenes assert assembly clearances** (a part-level headroom spec once broke an intentionally-short scene). Kinematic fit-proofs use `require_motion` (boolean sweep along a path, recorded for viewer scrubbing in motion mode); leak-proofs use `require_flow` (voxel void flood fill — check the WORST-CASE sagged pose via `objs=`, not nominal).
...
Per part: ~48 candidate orientations (principal + Fibonacci sphere) scored on **removability-weighted support cost** (support trapped in a bore ×10, via a voxel occupancy grid + escape rays) → top few get layer-sliced estimates → optional **ground-truth probe** (`probe=True`, the 🎯 toggle): the REAL slicer slices the finalists and re-ranks by exact support grams parsed from G-code feature blocks (`support_stats`), cached by mesh hash. Ranking currency follows the strategy (material=grams, fastest=minutes, plates=footprint); the heuristic ranks by **support VOLUME** (area × drop height, not raw area), **bed-fit and a size-scaled STABILITY gate outrank everything** (an orientation balanced on an edge — contact < max(60mm², 4% of footprint) — loses to any stable one before material is considered), and a non-principal tilt must beat the best flat orientation by >25% to win. A forced per-part orientation overrides the *selection* only — the full candidate field still gets scored so the compare-&-choose gallery survives. Packing is FFD shelves + a greedy **swap-to-fit** pass over each part's Pareto-set of orientations (never trades away a print hint). Plates export as STL/3MF/G-code; per-part `orient` override; split suggestions for support-heavy parts.
